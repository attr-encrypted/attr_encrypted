...
You can pass Initial Values to the underlying encryption algorithm as the <tt>:iv</tt> option.

The value can be a string or a Proc.

The :iv value is saved in the encrypted value and extracted during decryption so that :iv's may vary between encryptions of the same field.

For example, as a way to create randomly generated, 128 bit keys for AES-256-CBC:

  class SomeClass
    attr_encrypted :field, :key => App31::Application.config.secret_token, :marshal => true,
